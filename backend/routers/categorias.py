...
from db.schemas.categoria import categorias_schema
...
async def listar_categorias():
    cur = db_client.categorias.find(
        {},
        {"nombre": 1, "nivel": 1}
    ).sort([("nivel", 1), ("nombre", 1)])

    return categorias_schema(list(cur))
...
async def crear_categoria(payload: CategoriaCreate):
    exists = db_client.categorias.find_one(
        {
            "$or": [
    ...
    if exists:
        raise HTTPException(
            status_code=400,
            detail="Ya existe una categoría con ese nombre o nivel",
        )
...
async def modificar_categoria(categoria_id: str, payload: CategoriaUpdate):
    ...
    res = db_client.categorias.update_one(
...
async def eliminar_categoria(categoria_id: str):
    ...
    # Limpiamos referencias en users.categoria
