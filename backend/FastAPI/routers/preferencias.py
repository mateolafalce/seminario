...
async def obtener_preferencias(user: dict = Depends(current_user)):
    ...
    for preferencia in preferencias_cursor:
        ...
        preferencias_list.append({
            "id": str(preferencia["_id"]), # Devolver el ID para poder eliminarla
            "dias": dias_nombres,
...
async def modificar_preferencia(preferencia_id: str, preferencias: PreferenciasInput, user: dict = Depends(current_user)):
    ...
    if not preferencias.dias or not preferencias.horarios or not preferencias.canchas:
        raise HTTPException(status_code=400, detail="Una preferencia debe tener al menos un día, un horario y una cancha.")
    ...
    if not preferencia_a_modificar:
        raise HTTPException(status_code=404, detail="Preferencia no encontrada o no tienes permiso para modificarla")
    ...
    dias_oid = [dias_oid_map[dia] for dia in preferencias.dias if dia in dias_oid_map]
    horarios_oid = [horarios_oid_map[hora] for hora in preferencias.horarios if hora in horarios_oid_map]
    canchas_oid = [canchas_oid_map[cancha] for cancha in preferencias.canchas if cancha in canchas_oid_map]
...
async def eliminar_preferencia(preferencia_id: str, user: dict = Depends(current_user)):
    ...
    if not preferencia_a_eliminar:
        raise HTTPException(status_code=404, detail="Preferencia no encontrada o no tienes permiso para eliminarla")
