...
@router.post("/crear")
async def crear_cancha(cancha: CanchaInput, user: dict = Depends(current_user)):
...
async def eliminar_cancha(cancha_id: str, user: dict = Depends(current_user)):
    ...
    if not admin_data:
        raise HTTPException(status_code=403, detail="Solo los admin pueden eliminar canchas")
...
async def modificar_cancha(cancha_id: str, data: dict = Body(...), user: dict = Depends(current_user)):
    user_id = user.get("id") if isinstance(user, dict) else user.id
    user_db_data = await asyncio.to_thread(
        lambda: db_client.users.find_one({"_id": ObjectId(user_id)})
    )
    ...
    if not admin_data:
        raise HTTPException(status_code=403, detail="Solo los admin pueden modificar canchas")
    nuevo_nombre = data.get("nombre")
    if not nuevo_nombre:
        raise HTTPException(status_code=400, detail="El nombre es obligatorio")
    # Validar nombre único
    if db_client.canchas.find_one({"nombre": nuevo_nombre, "_id": {"$ne": ObjectId(cancha_id)}}):
        raise HTTPException(status_code=400, detail="Ya existe una cancha con ese nombre")
