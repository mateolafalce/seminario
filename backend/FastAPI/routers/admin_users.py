...
async def users(id: str, token_data: User = Depends(current_user)):
    return search_user("_id", ObjectId(id))


@router_admin.post("/add_admin", response_model=User,
                   status_code=status.HTTP_201_CREATED)
async def admin(admin: UserDB, current_admin: User = Depends(current_user)):
...
@router_admin.delete("/delete_admin/{id}",
                     status_code=status.HTTP_204_NO_CONTENT)
async def admin(id: str, current_admin: User = Depends(current_user)):

    # Validar que el admin no se esté eliminando a sí mismo
    ...
    if not admin_delete.deleted_count:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No se pudo eliminar al usuario - usuario no encontrado"
        )
